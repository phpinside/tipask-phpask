...
<? } } ?>
            ...
            <div class="description clearfix"><?=$question['description']?></div> 
...
<? } } ?>
                        ...
                        <? if(0!=$question['authorid'] && ($question['authorid']==$user['uid']) ) { ?>                        <div class="anscontent mt15">
                            ...
                            <? if(!$answer['tags'] || (count($answer['tags'])%2) == 0) { ?>                            <input type="button" class="button_4" value="继续追问" onclick="javascript:document.location = '<?=SITE_URL?>?question/appendanswer/<?=$question['id']?>/<?=$answer['id']?>.html'" />
                            <? } ?>                        </div>
                        <? } ?>                        <? if($answer['status'] && ($answer['authorid']==$user['uid']) ) { ?>                        <div class="anscontent mt15">
                            <input type="button" class="button_4" value="修改答案"  onclick="javascript:document.location = '<?=SITE_URL?>?question/editanswer/<?=$answer['id']?>/0.html'" />
                            <? if($answer['tags']!='' && (count($answer['tags'])%2) != 0) { ?>                            <input type="button"  value="继续回答" name="submit"  onclick="javascript:document.location = '<?=SITE_URL?>?question/appendanswer/<?=$question['id']?>/<?=$answer['id']?>/1.html'" class="button_4"/>
                            <? } ?>                        </div>
                        <? } ?>                    </div>
                    <div class="clr"></div>
